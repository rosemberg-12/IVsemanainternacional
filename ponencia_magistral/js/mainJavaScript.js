...
$(function(){
		...
		// Di&aacute;logo confirmación de eliminación
		...
		$('body').on('click','#listaUsuariosOK a',function (e){
			...
			idPonencia_ok = $(this).attr('src');
			accion_ok = $(this).attr('data-accion');

			$('#id_user').val(idUser_ok);
			$('#id_ponencia').val(idPonencia_ok);
			...
			if( accion_ok == 'editar'){
				...
				$('#identificacion').val($(this).parent().parent().children('td:eq(1)').text());
				$('#email').val($(this).parent().parent().children('td:eq(2)').text());
				$('#facultad option[value='+ $(this).parent().parent().children('td:eq(4)').text() +']').attr('selected',true);
				$('#grupo option[value='+ $(this).parent().parent().children('td:eq(5)').text() +']').attr('selected',true);


				// Seleccionar status
				$('#usr_tipo option[value='+ $(this).parent().parent().children('td:eq(6)').text() +']').attr('selected',true);
